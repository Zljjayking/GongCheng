//
//  DDLiveManagerMoreAgencySelectVC.m
//  GongChengDD
//
...
#import "DDLiveManagerMoreAgencySelectVC.h"
#import "MJRefresh.h"
...
#import "DDLiveManagerMoreAddApplyVC.h"//现场管理员继续教育添加报名页面
#import "DDLiveManagerMoreAddApplyRecordVC.h"//添加报名记录页面

@interface DDLiveManagerMoreAgencySelectVC ()<UITableViewDelegate,UITableViewDataSource,DDActionSheetViewDelegate,DDLocationMangerDelegate,DDGeocoderMangerDelegate>
...
@implementation DDLiveManagerMoreAgencySelectVC
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_trainCenterList params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        NSLog(@"**********现场管理员继续教育机构选择列表数据***************%@",responseObject);
...
-(void)judgeHasSignUp:(NSString *)agencyId andAgencyName:(NSString *)agencyName{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_judgeHasSignUp params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"***********判断是否存在现场管理员继续教育报名记录***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if ([responseObject[KData] isEqual:@1]) {//跳转到之前添过的人的列表
                DDLiveManagerMoreAddApplyRecordVC *liveManagerMoreAddApplyRecord=[[DDLiveManagerMoreAddApplyRecordVC alloc]init];
                liveManagerMoreAddApplyRecord.agencyId=agencyId;
                liveManagerMoreAddApplyRecord.agencyName=agencyName;
                [weakSelf.navigationController pushViewController:liveManagerMoreAddApplyRecord animated:YES];
            }
            else{//跳转到填信息页面
                DDLiveManagerMoreAddApplyVC *liveManagerMoreAddApply=[[DDLiveManagerMoreAddApplyVC alloc]init];
                liveManagerMoreAddApply.agencyId=agencyId;
                liveManagerMoreAddApply.agencyName=agencyName;
                [weakSelf.navigationController pushViewController:liveManagerMoreAddApply animated:YES];
            }
