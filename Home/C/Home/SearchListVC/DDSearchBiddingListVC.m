...

{
    ...
    //DDAreaSelectTableView *_areaSelectTableView;//区域筛选视图
    
    BOOL _isMoneySelected;//判断是否点开了金额筛选视图
    //DDMoneySelectView *_moneySelectView;//金额筛选视图
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendPostRequest:KHttpRequest_queryHighLightList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********查中标搜索结果数据***************%@",responseObject);
            ...
            if (listArr.count!=0) {
                ...
                for (NSDictionary *dic in listArr) {
                    ...
                    if (![DDUtils isEmptyString:model.title]) {
                        NSString *titleStr=[NSString stringWithFormat:@"<font color='#111111'>%@</font>",model.title];
                    ...
                    if (![DDUtils isEmptyString:model.winBidOrg]) {
                        NSString *winBidStr=[NSString stringWithFormat:@"<font color='#25A5FE'>%@</font>",model.winBidOrg];
                        ...
                        model.winBidString=winBid;
                    }
                    
                    if (![DDUtils isEmptyString:model.projectManager]) {
                        NSString *nameStr=[NSString stringWithFormat:@"<div style='text-align:center;'><font color='#25a5fe'>%@</font></div>",model.projectManager];
                        ...
                        model.peopleString=peopleString;
                    }
                    else{
                        NSString *nameStr=@"<div style='text-align:center;'><font color='#25a5fe'>-</font></div>";
                        NSAttributedString *peopleString = [[NSAttributedString alloc] initWithData:[nameStr dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType} documentAttributes:nil error:nil];
                        model.peopleString=peopleString;
                    }
                    ...
                    if (![DDUtils isEmptyString:model.amount]) {
                        if([model.amount isEqualToString:@"0.00"]){
                            ...
                            model.moneyString=[NSString stringWithFormat:@"%.2f万",model.amount.floatValue/10000];
                        }
                    }
                    else{
                        model.moneyString=@"-";
                    ...
                    model.timeString=[DDUtils getDateLineByStandardTime:model.publishDate];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        ...
        if ([model.type isEqualToString:@"1"]) {//中标人
        ...
        if ([model.type isEqualToString:@"1"]) {//中标价
        ...
        if ([model.type isEqualToString:@"1"]) {//中标时间
            cell.launchLab.text=@"中标时间";
        }
        else{//发布时间
            cell.launchLab.text=@"发布时间";
        }
