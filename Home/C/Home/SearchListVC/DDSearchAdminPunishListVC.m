...
#import "DDSearchAdminPunishListVC.h"
...
@interface DDSearchAdminPunishListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,AreaSelectTableViewDelegate,DateSelectViewDelegate>
...
@implementation DDSearchAdminPunishListVC
...
- (void)viewDidLoad {
    ...
    [MobClick endEvent:main_xingzhengchufa];
    _region=@"";//地区筛选
    _date=@"0";//日期筛选
    _isCitySelected=NO;
    _isDateSelected=NO;
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_searchAdminPunishList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********行政处罚搜索结果数据***************%@",responseObject);
            ...
            if (listArr.count!=0) {
                ...
                for (NSDictionary *dic in listArr) {
                    ...
                    if (![DDUtils isEmptyString:model.punish_name]) {
                        NSString *titleStr=[NSString stringWithFormat:@"<font color='#222222'>%@</font>",model.punish_name];
                        ...
                        model.punishTitleStr=attributeStr;
                    }
                ...
                [_noResultView showNoResultViewWithTitle:@"行政处罚信息" andImage:@"noResult_info"];
            }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        ...
        cell.serveContentLab.attributedText = model.punishTitleStr;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        //存浏览历史
        //    NSAttributedString *attributeStr = [[NSAttributedString alloc] initWithData:[model.punish_name dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType} documentAttributes:nil error:nil];
        //    [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:[DDUtils transformAttributedText:attributeStr] andGlobalType:nil andTransId:model.punish_id];
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    DDSearchAdminPunishListModel *model=_dataSourceArr[section];
...
-(void)companyClick:(UIButton *)sender{
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        DDSearchAdminPunishListModel *model=_dataSourceArr[sender.tag-150];
