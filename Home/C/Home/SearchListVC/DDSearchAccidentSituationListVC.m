//
//  DDSearchAccidentSituationListVC.m
//  GongChengDD
//
...
#import "DDSearchAccidentSituationListVC.h"
...
#import "DDSearchAccidentSituationListModel.h"//model
...
@interface DDSearchAccidentSituationListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,AreaSelectTableViewDelegate,DateSelectViewDelegate>
...
@implementation DDSearchAccidentSituationListVC
...
- (void)viewDidLoad {
    ...
    [MobClick event:main_shiguqingkuang];
    _region=@"";//地区筛选
    _date=@"0";//日期筛选
    _isCitySelected=NO;
    _isDateSelected=NO;
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_searchAdminPunishList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********事故情况搜索结果数据***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if (listArr.count!=0) {
                ...
                for (NSDictionary *dic in listArr) {
                    DDSearchAccidentSituationListModel *model = [[DDSearchAccidentSituationListModel alloc]initWithDictionary:dic error:nil];
                ...
                [_noResultView showNoResultViewWithTitle:@"事故情况" andImage:@"noResult_info"];
            }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        DDSearchAccidentSituationListModel *model=_dataSourceArr[indexPath.section];
        ...
        cell.deptLab2.textColor=kColorBlue;
        
        cell.serveContentLab.attributedText=model.titleStr;
        cell.serveContentLab.font=kFontSize34;
        ...
        //[cell loadDataWithContent:model.accident_title andDept:model.staff_name andTime:model.accident_issue_time];
...
-(void)peopleBtnClick:(UIButton *)sender{
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        [self presentLoginVCWithPeopleBtnSender:sender];
    }
    else{
        DDSearchAccidentSituationListModel *model=_dataSourceArr[sender.tag];
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    DDSearchAccidentSituationListModel *model=_dataSourceArr[section];
    ...
    label.attributedText = model.enterpriseStr;
...
-(void)companyClick:(UIButton *)sender{
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        DDSearchAccidentSituationListModel *model=_dataSourceArr[sender.tag-150];
...
#pragma mark 弹出登录注册页面
- (void)presentLoginVCWithPeopleBtnSender:(UIButton *)sender{
    DDLoginVC * vc = [[DDLoginVC alloc] init];
    vc.loginSuccessBlock = ^{
        //__weak __typeof(self) weakSelf=self;
        //[weakSelf requestTypesData];
