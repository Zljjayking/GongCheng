...
#import "DDSearchExcutedPeopleListVC.h"
#import "MJRefresh.h"
...
#import "DDLoseCreditPeopleCell.h"//cell
#import "DDSerarchExcutedPeopleListModel.h"//model

#import "DDExcutedPeopleDetailVC.h"//被执行人详情页面
#import "DDLoseCreditDetailVC.h"//失信信息详情页面
#import "DDUMengEventDefines.h"

@interface DDSearchExcutedPeopleListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,ProvinceSelectViewDelegate,DateSelectViewDelegate>

{
    ...
    UILabel *_label2;//放右边那个日期选择文字
    //UIImageView *_imgView2;//放右边那个日期选择小箭头
...
@property (nonatomic,strong) UIImageView *imgView2;//放右边那个日期选择小箭头
@property (nonatomic,strong) DDPublishDateSelectView *dateSelectView;//日期筛选视图

@end

@implementation DDSearchExcutedPeopleListVC
...
- (void)viewDidLoad {
    ...
    if ([self.menuId isEqualToString:@"86"]) {//失信信息
        [MobClick event:main_shixinxinxi];
    }
    else{
        [MobClick event:main_beizhixingren];
    }
    _region=@"";//地区筛选
    _date=@"0";//日期筛选
    _isCitySelected=NO;
...
- (void)requestData{
    ...
    [params setValue:self.nameText forKey:@"name"];
    //[params setValue:self.numberText forKey:@"executePersonCode"];
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_searchExcutedPeopleList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********被执行人和失信信息搜索结果数据***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if (listArr.count!=0) {
                ...
                for (NSDictionary *dic in listArr) {
                    DDSerarchExcutedPeopleListModel *model = [[DDSerarchExcutedPeopleListModel alloc]initWithDictionary:dic error:nil];
                ...
                if ([self.menuId isEqualToString:@"86"]) {
                    [_noResultView showNoResultViewWithTitle:@"失信信息" andImage:@"noResult_info"];
                }
                else{
                    [_noResultView showNoResultViewWithTitle:@"被执行人信息" andImage:@"noResult_info"];
                }
            }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        DDSerarchExcutedPeopleListModel *model=_dataSourceArr[indexPath.section];
        
        if ([self.menuId isEqualToString:@"86"]) {//失信信息
            static NSString * cellID = @"DDLoseCreditPeopleCell";
            DDLoseCreditPeopleCell * cell = (DDLoseCreditPeopleCell *)[tableView dequeueReusableCellWithIdentifier:cellID];
            if (cell == nil) {
            ...
            cell.nameLab.attributedText = model.nameAttriStr;
            cell.nameLab.font=kFontSize34;
            
            cell.numLab2.text=model.execute_case_number;
            cell.courtLab2.text=model.execute_court;
            
            cell.timeLab1.text=@"发布时间:";
            cell.timeLab2.text=model.execute_publish_date;
            ...
            cell.numLab2.text=model.execute_case_number;
            cell.aimLab2.text=model.execute_standard;
            cell.courtLab2.text=model.execute_court;
            
            cell.timeLab1.text=@"立案时间:";
            cell.timeLab2.text=model.execute_create_date;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        //此时需要存数据库了
        //存最近搜索
        [DDSearchHistoryDAOAndDB insertRecentSearchByTypeId:self.menuId andSearchText:self.nameText];
        //存浏览历史
        ...
        if ([self.menuId isEqualToString:@"86"]) {//失信信息
            ...
            DDExcutedPeopleDetailVC *excutedDetail=[[DDExcutedPeopleDetailVC alloc]init];
            excutedDetail.execute_id=model.execute_id;
            [self.navigationController pushViewController:excutedDetail animated:YES];
        }
