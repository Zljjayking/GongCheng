...
#import "DDJudgePaperDetailModel.h"//model
#import "DDProjectCheckOriginWebVC.h"//查看原文页面

@interface DDJudgePaperDetailVC ()<UITableViewDelegate,UITableViewDataSource>

{
    DDJudgePaperDetailModel *_model;
}
...
@implementation DDJudgePaperDetailVC
...
-(void)checkPaperClick{
    ...
    projectCheckOriginWebVC.hostUrl=_model.judgmentOriginalImg;
...
-(void)requestData{
    ...
    [params setValue:self.judgment_id forKey:@"judgmentId"];
    
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_judgePaperDetailByID params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        NSLog(@"**********裁判文书详情数据***************%@",responseObject);
        
        DDHttpResponse * response = [DDHttpResponse initWithResponseDic:responseObject];
        if (response.isSuccess) {
            [_loadingView hiddenLoadingView];
            _model=[[DDJudgePaperDetailModel alloc]initWithDictionary:responseObject[KData] error:nil];
        }
        else{
    ...
    if (indexPath.section==0) {
        if (indexPath.row==0 || indexPath.row==4) {
            ...
            if (indexPath.row==0) {
                ...
                cell.textView.text=_model.judgmentTitle;
            }
            else{
                //NSString *titleStr=[NSString stringWithFormat:@"<font color='#222222'>%@</font>",_model.judgmentExplain];
                NSAttributedString *attributeStr = [[NSAttributedString alloc] initWithData:[_model.judgmentExplain dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType} documentAttributes:nil error:nil];
            ...
            if (indexPath.row==1) {
                cell.titleLab.text=@"判决案号";
                cell.detailLab.text=_model.judgmentCaseNumber;
            }
            else if (indexPath.row==2){
                cell.titleLab.text=@"执行法院";
                cell.detailLab.text=_model.court;
            }
            else{
                cell.titleLab.text=@"发布日期";
                cell.detailLab.text=_model.judgmentPublishDate;
            }
