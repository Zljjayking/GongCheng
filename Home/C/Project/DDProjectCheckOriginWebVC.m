...
#import "DDProjectCheckOriginWebVC.h"

@interface DDProjectCheckOriginWebVC ()<WKNavigationDelegate,WKUIDelegate>
...
@implementation DDProjectCheckOriginWebVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.backgroundColor = kColorBackGroundColor;
    self.title = _hostTitle;
    ...
    self.webView= [[WKWebView alloc] initWithFrame:CGRectMake(0,0,Screen_Width, Screen_Height-KNavigationBarHeight-KHomeIndicatorHeight)];
    
    //添加KVO，WKWebView有一个属性estimatedProgress，就是当前网页加载的进度，所以监听这个属性。
    [self.webView addObserver:self forKeyPath:@"estimatedProgress" options:NSKeyValueObservingOptionNew context:nil];
    
    NSMutableURLRequest *resquest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:self.hostUrl]];
