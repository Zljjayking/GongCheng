//
//  DDSearchCivilAndGreenGroundListVC.m
//  GongChengDD
//
...
#import "DDSearchCivilAndGreenGroundListVC.h"
#import "MJRefresh.h"
...
#import "DDSearchCivilAndGreenGroundListCell.h"//cell
#import "DDSearchCivilAndGreenGroundListModel.h"//model

#import "DDSearchHistoryDAOAndDB.h"//搜索历史数据库操作类
#import "DDCompanyDetailVC.h"//公司详情页面
#import "DDCivilAndGreenGroundDetailVC.h"//文明工地绿色工地详情页面
#import "DDUMengEventDefines.h"

@interface DDSearchCivilAndGreenGroundListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,AreaSelectTableViewDelegate,DateSelectViewDelegate>
...
@implementation DDSearchCivilAndGreenGroundListVC
...
- (void)requestData{
    ...
    if ([self.menuId isEqualToString:@"149"]) {
        [params setValue:@"1" forKey:@"type"];
    }
    else{
        [params setValue:@"2" forKey:@"type"];
    }
    [params setValue:_date forKey:@"period"];
    ...
    [[DDHttpManager sharedInstance]sendPostRequest:KHttpRequest_civilGreenGroundList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********文明工地绿色工地搜索结果数据***************%@",responseObject);
            ...
            if (![response isEmpty]) {
                ...
                if (listArr.count!=0) {
                    ...
                    for (NSDictionary *dic in listArr) {
                        DDSearchCivilAndGreenGroundListModel *model = [[DDSearchCivilAndGreenGroundListModel alloc]initWithDictionary:dic error:nil];
                        
                        
                        if (![DDUtils isEmptyString:model.title]) {
                            NSString *titleStr=[NSString stringWithFormat:@"<font color='#222222'>%@</font>",model.title];
                    ...
                    if ([self.menuId isEqualToString:@"149"]) {
                        [_noResultView showNoResultViewWithTitle:@"文明工地信息" andImage:@"noResult_info"];
                    }
                    else{
                        [_noResultView showNoResultViewWithTitle:@"绿色工地信息" andImage:@"noResult_info"];
                    }
                }
            }
            else{
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        DDSearchCivilAndGreenGroundListModel *model=_dataSourceArr[indexPath.section];
        
        static NSString * cellID = @"DDSearchCivilAndGreenGroundListCell";
        DDSearchCivilAndGreenGroundListCell * cell = (DDSearchCivilAndGreenGroundListCell *)[tableView dequeueReusableCellWithIdentifier:cellID];
        if (cell == nil) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        DDCivilAndGreenGroundDetailVC *civilAndGreenGroundDetail=[[DDCivilAndGreenGroundDetailVC alloc]init];
        civilAndGreenGroundDetail.passValueID=model.id;
        [self.navigationController pushViewController:civilAndGreenGroundDetail animated:YES];
    }
}
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    DDSearchCivilAndGreenGroundListModel *model=_dataSourceArr[section];
...
//    }
//    else{
//        DDSearchCivilAndGreenGroundListModel *model=_dataSourceArr[sender.tag-150];
//        
...
-(void)actionsheetDisappear:(DDAreaSelectTableView *)actionSheet andAreaStr:(NSString *)areaStr andRegionId:(NSString *)regionId{
    ...
    if ([areaStr containsString:@"直辖县"]) {
        ...
        NSString *regionStr=[areaStr substringFromIndex:(range.location+1)];
    ...
    NSString *area=areaStr;
    if ([area containsString:@"全省"]) {
        area=[area stringByReplacingOccurrencesOfString:@"全省" withString:@""];
    }
    else if ([area containsString:@"全市"]) {
        area=[area stringByReplacingOccurrencesOfString:@"全市" withString:@""];
    }
    else if ([area containsString:@"全区"]) {
        area=[area stringByReplacingOccurrencesOfString:@"全区" withString:@""];
    }
...
- (void)presentLoginVCWithIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.loginSuccessBlock = ^{
        ...
        DDCivilAndGreenGroundDetailVC *civilAndGreenGroundDetail=[[DDCivilAndGreenGroundDetailVC alloc]init];
        //civilAndGreenGroundDetail.reward_id=model.reward_id;
        [self.navigationController pushViewController:civilAndGreenGroundDetail animated:YES];
    };
