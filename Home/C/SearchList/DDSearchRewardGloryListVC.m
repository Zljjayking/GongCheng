...
#import "DDSearchRewardGloryListVC.h"
#import "MJRefresh.h"
...
#import "DDRewardTypeSelectView.h"//奖项类别选择的View
...
@interface DDSearchRewardGloryListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,AreaSelectTableViewDelegate,DateSelectViewDelegate,DDRewardTypeSelectViewDelegate>

{
    ...
    UILabel *_label2;//放中间那个发布日期选择文字
    UILabel *_label3;//放右边那个奖项类别选择文字
    ...
    BOOL _isTypeSelected;//判断是否点开了奖项类别筛选视图
    
    NSString *_region;//地区筛选
    NSString *_date;//日期筛选
    NSString *_type;//奖项类别筛选
}
...
@property (nonatomic,strong) UIImageView *imgView2;//放中间那个资质等级选择小箭头
@property (nonatomic,strong) DDPublishDateSelectView *dateSelectView;//日期筛选视图
@property (nonatomic,strong) UIImageView *imgView3;//放右边那个奖项类别选择小箭头
@property (nonatomic,strong) DDRewardTypeSelectView *typeSelectView;//奖项类别筛选视图

@end

@implementation DDSearchRewardGloryListVC
...
-(void)createChooseBtns{
    ...
    //日期筛选按钮
    UIButton *moneySelectBtn=[[UIButton alloc]initWithFrame:CGRectMake(Screen_Width/3, 0, Screen_Width/3, 39)];
    ...
    CGRect middleTextFrame = [@"发布日期" boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFontSize30} context:nil];
    CGFloat middleWidth=middleTextFrame.size.width+4+15;
    if (middleWidth>=(Screen_Width/3-40)) {
    ...
    //奖项类别筛选按钮
    UIButton *typeSelectBtn=[[UIButton alloc]initWithFrame:CGRectMake(Screen_Width/3*2, 0, Screen_Width/3, 39)];
    [typeSelectBtn setBackgroundColor:kColorWhite];
    
    _label3=[[UILabel alloc]init];
    _label3.text=@"奖项类别";
    _label3.textColor=KColorBlackTitle;
    _label3.font=kFontSize30;
    [typeSelectBtn addSubview:_label3];
    
    _imgView3=[[UIImageView alloc]init];
    _imgView3.contentMode = UIViewContentModeScaleAspectFit;
    _imgView3.image=[UIImage imageNamed:@"home_search_down"];
    [typeSelectBtn addSubview:_imgView3];
    [typeSelectBtn addTarget:self action:@selector(typeSelectClick) forControlEvents:UIControlEventTouchUpInside];
    
    CGRect rightTextFrame = [@"奖项类别" boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFontSize30} context:nil];
    ...
    [self.view addSubview:typeSelectBtn];
...
- (void)requestData{
    ...
    [params setValue:_type forKey:@"reward_group"];
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_searchAdminPunishList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********获奖荣誉搜索结果数据***************%@",responseObject);
            ...
            if (![response isEmpty]) {
                ...
                if (listArr.count!=0) {
                    ...
                    for (NSDictionary *dic in listArr) {
                        DDSearchRewardGloryListModel *model = [[DDSearchRewardGloryListModel alloc]initWithDictionary:dic error:nil];
                        ...
                        if (![DDUtils isEmptyString:model.staff_name]) {
                            ...
                            model.titleNameStr=attributeStr;
                        }
                        else{
                    ...
                    [_noResultView showNoResultViewWithTitle:@"获奖荣誉信息" andImage:@"noResult_info"];
                }
            }
            else{
                [_noResultView showNoResultViewWithTitle:@"获奖荣誉信息" andImage:@"noResult_info"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        ...
        cell.nameLab.attributedText = model.titleNameStr;
        cell.nameLab.font=kFontSize34;
        
        cell.rewardLab2.text=model.reward_type;
        cell.deptLab2.text=model.executor_defendant;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        //存浏览历史
        //    NSAttributedString *attributeStr = [[NSAttributedString alloc] initWithData:[model.reward_explain dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType} documentAttributes:nil error:nil];
        //    [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:[DDUtils transformAttributedText:attributeStr] andGlobalType:nil andTransId:model.reward_id];
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    DDSearchRewardGloryListModel *model=_dataSourceArr[section];
...
//    }
//    else{
//        DDSearchRewardGloryListModel *model=_dataSourceArr[sender.tag-150];
//        
...
-(void)actionsheetDisappear:(DDPublishDateSelectView *)actionSheet andDateCode:(NSString *)dateCode andDateStr:(NSString *)dateStr{
    ...
    CGRect middleTextFrame = [dateStr boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFontSize30} context:nil];
    CGFloat middleWidth=middleTextFrame.size.width+4+15;
    if (middleWidth>=(Screen_Width/3-40)) {
        _label2.frame=CGRectMake(20, 12, (Screen_Width/3-40)-4-15, 15);
        _imgView2.frame=CGRectMake(CGRectGetMaxX(_label2.frame)+4, 12, 15, 15);
...
-(void)typeSelectClick{
    if (_isTypeSelected==NO) {
        ...
        _typeSelectView=[[DDRewardTypeSelectView alloc]initWithFrame:CGRectMake(0, 39+60, Screen_Width, Screen_Height-KNavigationBarHeight-39-60)];
        _typeSelectView.attachHeight=@"0";
        _typeSelectView.typeCode=_type;
        __weak __typeof(self) weakSelf=self;
...
#pragma mark DDRewardTypeSelectViewDelegate代理回调
-(void)actionsheetDisappear:(DDRewardTypeSelectView *)actionSheet andTypeStr:(NSString *)typeStr andTypeCode:(NSString *)typeCode{
    _type=typeCode;
