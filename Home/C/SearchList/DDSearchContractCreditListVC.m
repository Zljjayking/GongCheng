//
//  DDSearchContractCreditListVC.m
//  GongChengDD
//
...
#import "DDSearchContractCreditListVC.h"
#import "MJRefresh.h"
...
#import "DDDeliverAgencySelectView.h"//机构选择View

#import "DDSearchContractCreditListCell.h"//cell
#import "DDSearchContractCreditListModel.h"//model
...
@interface DDSearchContractCreditListVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,AreaSelectTableViewDelegate,DDDeliverAgencySelectViewDelegate>
...
@property (nonatomic,strong) DDDeliverAgencySelectView *agencySelectView;//机构选择视图

@end

@implementation DDSearchContractCreditListVC
...
-(void)popBackClick{
    ...
    [_agencySelectView hiddenActionSheet];
...
-(void)createChooseBtns{
    ...
    CGRect rightTextFrame = [@"发证级别" boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFontSize30} context:nil];
...
- (void)requestData{
    ...
    [params setValue:_agency forKey:@"certificationLevel"];
    ...
    [[DDHttpManager sharedInstance]sendPostRequest:KHttpRequest_queryHighLightList params:params success:^(NSURLSessionDataTask *operation, id responseObject)  {
        NSLog(@"**********守合同重信用搜索结果数据***************%@",responseObject);
            ...
            if (![response isEmpty]) {
                ...
                if (listArr.count!=0) {
                    ...
                    for (NSDictionary *dic in listArr) {
                        DDSearchContractCreditListModel *model = [[DDSearchContractCreditListModel alloc]initWithDictionary:dic error:nil];
                        ...
                        if (![DDUtils isEmptyString:model.tccTitle]) {
                            NSString *titleStr=[NSString stringWithFormat:@"<font color='#222222'>%@</font>",model.tccTitle];
                            NSAttributedString *title = [[NSAttributedString alloc] initWithData:[titleStr dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType} documentAttributes:nil error:nil];
                    ...
                    [_noResultView showNoResultViewWithTitle:@"守合同重信用信息" andImage:@"noResult_info"];
                }
            }
            else{
                [_noResultView showNoResultViewWithTitle:@"守合同重信用信息" andImage:@"noResult_info"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_dataSourceArr.count>0) {
        DDSearchContractCreditListModel *model=_dataSourceArr[indexPath.section];
        
        static NSString * cellID = @"DDSearchContractCreditListCell";
        DDSearchContractCreditListCell * cell = (DDSearchContractCreditListCell *)[tableView dequeueReusableCellWithIdentifier:cellID];
        if (cell == nil) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
        ...
        //存浏览历史
        NSAttributedString *attributeStr = [[NSAttributedString alloc] initWithData:[model.tccTitle dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType} documentAttributes:nil error:nil];
        ...
        abideContractDetail.enterpriseId=model.enterpriseId;
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    DDSearchContractCreditListModel *model=_dataSourceArr[section];
...
//    }
//    else{
//        DDSearchContractCreditListModel *model=_dataSourceArr[sender.tag-150];
//        
...
-(void)moneySelectClick{
    if (_isDateSelected==NO) {
        ...
        _agencySelectView=[[DDDeliverAgencySelectView alloc]initWithFrame:CGRectMake(0, 39+60, Screen_Width, Screen_Height-KNavigationBarHeight-39-60)];
        _agencySelectView.attachHeight=@"0";
        _agencySelectView.agencyCode=_agency;
        __weak __typeof(self) weakSelf=self;
        _agencySelectView.hiddenBlock = ^{
            weakSelf.imgView2.image=[UIImage imageNamed:@"home_search_down"];
            
            [weakSelf.agencySelectView hiddenActionSheet];
            
            _isDateSelected=NO;
        };
        _agencySelectView.delegate=self;
        [_agencySelectView show];
        [_textField resignFirstResponder];
...
-(void)actionsheetDisappear:(DDDeliverAgencySelectView *)actionSheet andName:(NSString *)name andCode:(NSString *)code{
    
    _agency=code;
    
    CGRect rightTextFrame = [name boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFontSize30} context:nil];
    CGFloat rightWidth=rightTextFrame.size.width+4+15;
    if (rightWidth>=(Screen_Width/2-40)) {
        _label2.frame=CGRectMake(20, 12, (Screen_Width/2-40)-4-15, 15);
        _imgView2.frame=CGRectMake(CGRectGetMaxX(_label2.frame)+4, 12, 15, 15);
