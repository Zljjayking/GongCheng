...
#import "DDSearchSafeCertiListVC.h"//安许证搜索列表页面（非全局搜索）
#import "DDSearchBusinessInfoListVC.h"//查工商搜索列表页面（非全局搜索）
#import "DDSearchCreditScoreListVC.h"//信用评分搜索列表页面（非全局搜索）
#import "DDSearchCivilAndGreenGroundListVC.h"//文明绿色工地搜索列表页面（非全局搜索）
#import "DDSearchConstructMethodListVC.h"//施工工法搜索列表页面（非全局搜索）
#import "DDSearchContractCreditListVC.h"//守合同重信用搜索列表页面（非全局搜索）
#import "DDSearchAAACertiListVC.h"//AAA证书搜索列表页面（非全局搜索）
#import "DDSearchManageSystemListVC.h"//管理体系搜索列表（非全局搜索）
#import "DDSearchBrandListVC.h"//查商标搜索列表（非全局搜索）
...
#import "DDCivilAndGreenGroundDetailVC.h"//文明绿色工地详情页面
#import "DDWorkingLawDetailVC.h"//施工工法详情页面
#import "DDAbideContractVC.h"//守合同重信用详情页面
#import "DDThreeACerDetailVC.h"//AAA证书详情页面
...
-(void)viewWillAppear:(BOOL)animated{
    if ([self.audioType isEqualToString:@"1"]) {//特例：从语音搜索跳过来的
        ...
        if(_textField.text.length>1){
            ...
            globalListVC.index=0;
            globalListVC.searchText=_textField.text;
            globalListVC.hidesBottomBarWhenPushed=YES;
...
-(void)editNavItem{
    ...
    _topBgView=[[UIView alloc]initWithFrame:CGRectMake(15, 4.5, Screen_Width-20, 35)];
    [self.navigationController.navigationBar addSubview:_topBgView];
    
    UIView *leftBgView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, Screen_Width-20-50, 35)];
    ...
    UIImageView *imageView=[[UIImageView alloc]initWithFrame:CGRectMake(10, 11, 13, 13)];
    imageView.image=[UIImage imageNamed:@"home_search"];
    [leftBgView addSubview:imageView];
    
    _textField=[[UITextField alloc]initWithFrame:CGRectMake(CGRectGetMaxX(imageView.frame)+5, 0, Screen_Width-20-50-30, 35)];
    ...
    [_textField setValue:kFontSize26 forKeyPath:@"_placeholderLabel.font"];
    ...
    UIButton *cancelBtn=[[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(leftBgView.frame), 0, 50, 35)];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    if (textField.text.length<2) {
        ...
        if ([self.type isEqualToString:@"1"]) {//表示从首页公共搜索进入
            ...
            else if ([self.menuId isEqualToString:@"147"]) {//PPP项目
                DDSearchBiddingListVC *searchBidding=[[DDSearchBiddingListVC alloc]init];
                searchBidding.searchText=textField.text;
                searchBidding.menuId=self.menuId;
                [self.navigationController pushViewController:searchBidding animated:NO];
            ...
            else if ([self.menuId isEqualToString:@"17"]) {//安许证
                DDSearchSafeCertiListVC *searchSafeCertiListVC=[[DDSearchSafeCertiListVC alloc]init];
                searchSafeCertiListVC.searchText=textField.text;
                searchSafeCertiListVC.menuId=self.menuId;
                [self.navigationController pushViewController:searchSafeCertiListVC animated:NO];
            }
            ...
            else  if ([self.menuId isEqualToString:@"123"]) {//查工商
                DDSearchBusinessInfoListVC *searchBusinessList=[[DDSearchBusinessInfoListVC alloc]init];
                searchBusinessList.menuId=self.menuId;
                searchBusinessList.searchText=textField.text;
                [self.navigationController pushViewController:searchBusinessList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"129"]) {//信用评分
                DDSearchCreditScoreListVC *creditScoreList=[[DDSearchCreditScoreListVC alloc]init];
                creditScoreList.menuId=self.menuId;
                creditScoreList.searchText=textField.text;
                [self.navigationController pushViewController:creditScoreList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"149"]) {//文明工地
                DDSearchCivilAndGreenGroundListVC *civilAndGreenList=[[DDSearchCivilAndGreenGroundListVC alloc]init];
                civilAndGreenList.menuId=self.menuId;
                civilAndGreenList.searchText=textField.text;
                [self.navigationController pushViewController:civilAndGreenList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"150"]) {//绿色工地
                DDSearchCivilAndGreenGroundListVC *civilAndGreenList=[[DDSearchCivilAndGreenGroundListVC alloc]init];
                civilAndGreenList.menuId=self.menuId;
                civilAndGreenList.searchText=textField.text;
                [self.navigationController pushViewController:civilAndGreenList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"128"]) {//施工工法
                DDSearchConstructMethodListVC *constructMethodList=[[DDSearchConstructMethodListVC alloc]init];
                constructMethodList.menuId=self.menuId;
                constructMethodList.searchText=textField.text;
                [self.navigationController pushViewController:constructMethodList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"127"]) {//守合同重信用
                DDSearchContractCreditListVC *contractCreditList=[[DDSearchContractCreditListVC alloc]init];
                contractCreditList.menuId=self.menuId;
                contractCreditList.searchText=textField.text;
                [self.navigationController pushViewController:contractCreditList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"126"]) {//AAA证书
                DDSearchAAACertiListVC *AAACertiList=[[DDSearchAAACertiListVC alloc]init];
                AAACertiList.menuId=self.menuId;
                AAACertiList.searchText=textField.text;
                [self.navigationController pushViewController:AAACertiList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"125"]) {//管理体系
                DDSearchManageSystemListVC *manageSystemList=[[DDSearchManageSystemListVC alloc]init];
                manageSystemList.menuId=self.menuId;
                manageSystemList.searchText=textField.text;
                [self.navigationController pushViewController:manageSystemList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"124"]) {//查商标
                DDSearchBrandListVC *brandList=[[DDSearchBrandListVC alloc]init];
                brandList.menuId=self.menuId;
                brandList.searchText=textField.text;
                [self.navigationController pushViewController:brandList animated:NO];
            }
            else  if ([self.menuId isEqualToString:@"138"] || [self.menuId isEqualToString:@"139"] || [self.menuId isEqualToString:@"140"] || [self.menuId isEqualToString:@"141"] || [self.menuId isEqualToString:@"142"] || [self.menuId isEqualToString:@"143"] || [self.menuId isEqualToString:@"144"] || [self.menuId isEqualToString:@"145"] || [self.menuId isEqualToString:@"146"]) {//建筑师:138  结构师:139  土木工程师:140 " "公用设备师:141  电气工程师:142  化工工程师:143  监理工程师:144  造价工程师:145  消防工程师:146；
                DDSearchBuilderAndManagerListVC *builderAndManager=[[DDSearchBuilderAndManagerListVC alloc]init];
                builderAndManager.searchText=textField.text;
                builderAndManager.menuId=self.menuId;
                [self.navigationController pushViewController:builderAndManager animated:NO];
        ...
        if ([textField.text isEqual:@""]) {
            ...
            if (textField.text.length<2) {
                ...
                if ([self.type isEqualToString:@"1"]) {//表示从首页公共搜索进入
                    DDGlobalListVC *globalListVC=[[DDGlobalListVC alloc]init];
                    globalListVC.index=_index;
                    globalListVC.searchStringBlock = ^(NSString *searchStr) {
    ...
    if (indexPath.section==1) {
        if (_isFromDB==YES) {//浏览历史（从数据库获取）（跳详情）
            ...
            if ([DDUtils isEmptyString:[DDUserManager sharedInstance].userid]) {
                ...
                DDSearchRecordModel *model=_dataSource[indexPath.row];
                if ([self.type isEqualToString:@"1"]) {//全局搜索的三种情况
                    if([model.globalType isEqualToString:@"1"]){//人员详情
...
//                    }
                    ...
                    if ([self.menuId isEqualToString:@"147"]) {//PPP项目详情页面（同项目详情页面）
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                    ...
                    if ([self.menuId isEqualToString:@"17"]) {//安许证详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDSafePermissionVC *safeCertiDetail=[[DDSafePermissionVC alloc]init];
                        safeCertiDetail.enterpriseId=model.transId;
                    ...
                    if ([self.menuId isEqualToString:@"123"]) {//查工商详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDBusinesslicenseVC *businessDetail=[[DDBusinesslicenseVC alloc]init];
                        businessDetail.enterpriseId=model.transId;
                        [self.navigationController pushViewController:businessDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"129"]) {//信用评分详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDCompanyCreditScoreListVC *creditScoreDetail=[[DDCompanyCreditScoreListVC alloc]init];
                        creditScoreDetail.enterpriseId=model.transId;
                        [self.navigationController pushViewController:creditScoreDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"149"]) {//文明工地详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDCivilAndGreenGroundDetailVC *civilAndGreenDetail=[[DDCivilAndGreenGroundDetailVC alloc]init];
                        civilAndGreenDetail.passValueID=model.transId;
                        [self.navigationController pushViewController:civilAndGreenDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"150"]) {//绿色工地详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                    ...
                    if ([self.menuId isEqualToString:@"128"]) {//施工工法详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDWorkingLawDetailVC *constructMethodDetail=[[DDWorkingLawDetailVC alloc]init];
                        constructMethodDetail.passValueID=model.transId;
                        [self.navigationController pushViewController:constructMethodDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"125"]) {//管理体系详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDManageDetailVC *manageDetail=[[DDManageDetailVC alloc]init];
                        manageDetail.urlid=model.transId;
                        [self.navigationController pushViewController:manageDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"126"]) {//AAA证书详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDThreeACerDetailVC *threeADetail=[[DDThreeACerDetailVC alloc]init];
                        threeADetail.passValueID=model.transId;
                        [self.navigationController pushViewController:threeADetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"127"]) {//守合同重信用详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDAbideContractVC *abideContractDetail=[[DDAbideContractVC alloc]init];
                        abideContractDetail.enterpriseId=model.transId;
                        [self.navigationController pushViewController:abideContractDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"124"]) {//商标详情页面
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
                        
                        DDBrandDetailVC *brandDetail=[[DDBrandDetailVC alloc]init];
                        brandDetail.urlid=model.transId;
                        [self.navigationController pushViewController:brandDetail animated:YES];
                    }
                    if ([self.menuId isEqualToString:@"138"] || [self.menuId isEqualToString:@"139"] || [self.menuId isEqualToString:@"140"] || [self.menuId isEqualToString:@"141"] || [self.menuId isEqualToString:@"142"] || [self.menuId isEqualToString:@"143"] || [self.menuId isEqualToString:@"144"] || [self.menuId isEqualToString:@"145"] || [self.menuId isEqualToString:@"146"]) {//建筑师:138  结构师:139  土木工程师:140 " "公用设备师:141  电气工程师:142  化工工程师:143  监理工程师:144  造价工程师:145  消防工程师:146；
                        [DDSearchHistoryDAOAndDB insertHistorySearchByTypeId:self.menuId andSearchResult:model.title andGlobalType:nil andTransId:model.transId];
                        [self getDataFromDB];
            ...
            if ([self.type isEqualToString:@"1"]) {//表示从首页公共搜索进入
                ...
                if ([self.menuId isEqualToString:@"147"]) {//PPP项目列表
                    DDSearchBiddingListVC *searchbidding=[[DDSearchBiddingListVC alloc]init];
                ...
                if ([self.menuId isEqualToString:@"24"]) {//行政处罚列表
                    ...
                    DDSearchSafeCertiListVC *searchSafeCertiListVC=[[DDSearchSafeCertiListVC alloc]init];
                    searchSafeCertiListVC.searchText=model.searchTitle;
                    searchSafeCertiListVC.menuId=self.menuId;
                    [self.navigationController pushViewController:searchSafeCertiListVC animated:NO];
                }
                else if ([self.menuId isEqualToString:@"9"]) {//买公司
                    ...
                    DDSearchBusinessInfoListVC *businessInfoList=[[DDSearchBusinessInfoListVC alloc]init];
                    businessInfoList.menuId=self.menuId;
                    businessInfoList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:businessInfoList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"129"]) {//信用评分
                    DDSearchCreditScoreListVC *creditScoreList=[[DDSearchCreditScoreListVC alloc]init];
                    creditScoreList.menuId=self.menuId;
                    creditScoreList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:creditScoreList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"149"]) {//文明工地
                    DDSearchCivilAndGreenGroundListVC *civilAndGreenList=[[DDSearchCivilAndGreenGroundListVC alloc]init];
                    civilAndGreenList.menuId=self.menuId;
                    civilAndGreenList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:civilAndGreenList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"150"]) {//绿色工地
                    DDSearchCivilAndGreenGroundListVC *civilAndGreenList=[[DDSearchCivilAndGreenGroundListVC alloc]init];
                    ...
                    DDSearchConstructMethodListVC *constructMethodList=[[DDSearchConstructMethodListVC alloc]init];
                    constructMethodList.menuId=self.menuId;
                    constructMethodList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:constructMethodList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"125"]) {//管理体系
                    DDSearchManageSystemListVC *manageSystemList=[[DDSearchManageSystemListVC alloc]init];
                    manageSystemList.menuId=self.menuId;
                    manageSystemList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:manageSystemList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"126"]) {//AAA证书
                    DDSearchAAACertiListVC *AAACertiList=[[DDSearchAAACertiListVC alloc]init];
                    AAACertiList.menuId=self.menuId;
                    AAACertiList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:AAACertiList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"127"]) {//守合同重信用
                    DDSearchContractCreditListVC *creditAndContractList=[[DDSearchContractCreditListVC alloc]init];
                    creditAndContractList.menuId=self.menuId;
                    creditAndContractList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:creditAndContractList animated:NO];
                }
                else if ([self.menuId isEqualToString:@"124"]) {//查商标
                    DDSearchBrandListVC *brandList=[[DDSearchBrandListVC alloc]init];
                    brandList.menuId=self.menuId;
                    brandList.searchText=model.searchTitle;
                    [self.navigationController pushViewController:brandList animated:NO];
                }
                if ([self.menuId isEqualToString:@"138"] || [self.menuId isEqualToString:@"139"] || [self.menuId isEqualToString:@"140"] || [self.menuId isEqualToString:@"141"] || [self.menuId isEqualToString:@"142"] || [self.menuId isEqualToString:@"143"] || [self.menuId isEqualToString:@"144"] || [self.menuId isEqualToString:@"145"] || [self.menuId isEqualToString:@"146"]) {//建筑师:138  结构师:139  土木工程师:140 " "公用设备师:141  电气工程师:142  化工工程师:143  监理工程师:144  造价工程师:145  消防工程师:146；
                    DDSearchBuilderAndManagerListVC *builderAndManager=[[DDSearchBuilderAndManagerListVC alloc]init];
...
-(void)allSearchRecentCellSelectButton:(DDAllSearchRecentCell *)allSearchRecentCell buttonIndex:(NSInteger)index{
    ...
    if ([self.type isEqualToString:@"1"]) {//表示从首页公共搜索进入
        DDGlobalListVC *globalListVC=[[DDGlobalListVC alloc]init];
        globalListVC.index=_index;
        globalListVC.globalListType = DDGlobalListTypeHistory;
        globalListVC.indexBlock = ^(NSInteger index) {
            _index=index;
        };
        ...
        if ([self.menuId isEqualToString:@"24"]) {//行政处罚列表
            ...
            DDSearchSafeCertiListVC *searchSafeCertiListVC=[[DDSearchSafeCertiListVC alloc]init];
            searchSafeCertiListVC.searchText=model.title;
            searchSafeCertiListVC.menuId=self.menuId;
            [self.navigationController pushViewController:searchSafeCertiListVC animated:NO];
        }
        else if ([self.menuId isEqualToString:@"9"]) {//买公司
            ...
            DDSearchBusinessInfoListVC *businessInfoList=[[DDSearchBusinessInfoListVC alloc]init];
            businessInfoList.menuId=self.menuId;
            businessInfoList.searchText=model.title;
            [self.navigationController pushViewController:businessInfoList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"129"]) {//信用评分
            DDSearchCreditScoreListVC *creditScoreList=[[DDSearchCreditScoreListVC alloc]init];
            creditScoreList.menuId=self.menuId;
            creditScoreList.searchText=model.title;
            [self.navigationController pushViewController:creditScoreList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"149"]) {//文明工地
            DDSearchCivilAndGreenGroundListVC *civilAndGreenList=[[DDSearchCivilAndGreenGroundListVC alloc]init];
            civilAndGreenList.menuId=self.menuId;
            civilAndGreenList.searchText=model.title;
            [self.navigationController pushViewController:civilAndGreenList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"150"]) {//绿色工地
            DDSearchCivilAndGreenGroundListVC *civilAndGreenList=[[DDSearchCivilAndGreenGroundListVC alloc]init];
            ...
            DDSearchConstructMethodListVC *constructMethodList=[[DDSearchConstructMethodListVC alloc]init];
            constructMethodList.menuId=self.menuId;
            constructMethodList.searchText=model.title;
            [self.navigationController pushViewController:constructMethodList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"125"]) {//管理体系
            DDSearchManageSystemListVC *manageSystemList=[[DDSearchManageSystemListVC alloc]init];
            manageSystemList.menuId=self.menuId;
            manageSystemList.searchText=model.title;
            [self.navigationController pushViewController:manageSystemList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"126"]) {//AAA证书
            DDSearchAAACertiListVC *AAACertiList=[[DDSearchAAACertiListVC alloc]init];
            AAACertiList.menuId=self.menuId;
            AAACertiList.searchText=model.title;
            [self.navigationController pushViewController:AAACertiList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"127"]) {//守合同重信用
            DDSearchContractCreditListVC *creditAndContractList=[[DDSearchContractCreditListVC alloc]init];
            creditAndContractList.menuId=self.menuId;
            creditAndContractList.searchText=model.title;
            [self.navigationController pushViewController:creditAndContractList animated:NO];
        }
        else if ([self.menuId isEqualToString:@"124"]) {//查商标
            DDSearchBrandListVC *brandList=[[DDSearchBrandListVC alloc]init];
            brandList.menuId=self.menuId;
            brandList.searchText=model.title;
            [self.navigationController pushViewController:brandList animated:NO];
        }
        if ([self.menuId isEqualToString:@"138"] || [self.menuId isEqualToString:@"139"] || [self.menuId isEqualToString:@"140"] || [self.menuId isEqualToString:@"141"] || [self.menuId isEqualToString:@"142"] || [self.menuId isEqualToString:@"143"] || [self.menuId isEqualToString:@"144"] || [self.menuId isEqualToString:@"145"] || [self.menuId isEqualToString:@"146"]) {//建筑师:138  结构师:139  土木工程师:140 " "公用设备师:141  电气工程师:142  化工工程师:143  监理工程师:144  造价工程师:145  消防工程师:146；
            DDSearchBuilderAndManagerListVC *builderAndManager=[[DDSearchBuilderAndManagerListVC alloc]init];
