//
//  DDBusinessLicenseChangeRecordVC.m
//  GongChengDD
//
//  Created by csq on 2018/6/19.
...
@interface DDBusinessLicenseChangeRecordVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation DDBusinessLicenseChangeRecordVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.view.backgroundColor = kColorBackGroundColor;
    self.navigationItem.title = @"营业执照变更记录";
...
//    [self.tableView setSeparatorColor:KColorTableSeparator];
    ...
    _headerLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, Screen_Width-20, 40)];
    _headerLabel.textColor = KColorGreySubTitle;
    _headerLabel.font = kFontSize30;
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [weakSelf requestDataWithHUD:hud];
    }];
}
...
//    MBProgressHUD * hud = [DDUtils showHUDCustom:@""];
    [[DDHttpManager sharedInstance] sendPostRequest:KHttpRequest_ecbusinesslicensechangeinfo params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        
        //NSLog(@"营业执照变更记录%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            //给_headerLabel赋值
            NSString * totalCountString = [NSString stringWithFormat:@"%d",[dataDict[KTotalCount] integerValue]];
            if (![DDUtils isEmptyString:totalCountString]) {
                NSString * totalString = [NSString stringWithFormat:@"共%@条",totalCountString];
                NSMutableAttributedString * attributedString =  [DDUtils adjustTextColor:totalString rangeText:totalCountString color:kColorRed];
                _headerLabel.attributedText = attributedString;
            }else{
                NSString * totalString = @"共0条";
                NSMutableAttributedString * attributedString =  [DDUtils adjustTextColor:totalString rangeText:@"0" color:kColorRed];
                _headerLabel.attributedText = attributedString;
            }
    ...
    [params setValue:@"15" forKey:@"limit"];
     [[DDHttpManager sharedInstance] sendPostRequest:KHttpRequest_ecbusinesslicensechangeinfo params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (response.isSuccess) {
            ...
            //给_headerLabel赋值
            NSString * totalCountString = [NSString stringWithFormat:@"%ld",[dataDict[KTotalCount] integerValue]];
            if (![DDUtils isEmptyString:totalCountString]) {
                NSString * totalString = [NSString stringWithFormat:@"共%@条",totalCountString];
                NSMutableAttributedString * attributedString =  [DDUtils adjustTextColor:totalString rangeText:totalCountString color:kColorRed];
                _headerLabel.attributedText = attributedString;
