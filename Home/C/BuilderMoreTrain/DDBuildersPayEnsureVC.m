...
#import "DDBuildersPayEnsureVC.h"
...
@interface DDBuildersPayEnsureVC ()<UITableViewDelegate,UITableViewDataSource,CNContactPickerDelegate,DDCitySelectPickerViewDelegate,DDPayMethodViewDelegate,PlaceholdTextViewDelegate>
...
@implementation DDBuildersPayEnsureVC
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            ...
            cell.titleLab2.text=[NSString stringWithFormat:@"二级建造师继续教育-%@",_buildType];
        }
        else if(indexPath.row==1){
            cell.titleLab1.text=@"费用金额";
            if (![DDUtils isEmptyString:self.majorPrice]) {
                cell.titleLab2.text=[NSString stringWithFormat:@"¥%d",[self.majorPrice integerValue]];
            }
            else{
                cell.titleLab2.text=@"";
            }
...
-(void)actionsheetDisappearAliPay:(DDPayMethodView *)actionSheet{
       ...
       if([_buildType isEqualToString:@"主项"]){
        ...
        if ([_postage isEqualToString:@"0"] || [DDUtils isEmptyString:_postage]) {
...
-(void)createPaySuccessView{
    ...
    if (self.isViewVisable) {
        ...
        if ([DDUtils isEmptyString:self.vcName]) {
            succeedVC.vcName=@"DDBuilderMoreTrainVC";
        }
        else{
