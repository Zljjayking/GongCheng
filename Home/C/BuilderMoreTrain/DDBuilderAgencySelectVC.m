...
#import "DDBuilderAgencySelectVC.h"
#import "MJRefresh.h"
...
#import "DDBuildersAndAgencyVC.h"//选中的人员和培训机构汇总页面
#import "DDBuilderAddApplyVC.h"//建造师添加报名页面
#import "DDBuilderAddApplyRecordVC.h"//添加报名记录页面

@interface DDBuilderAgencySelectVC ()<UITableViewDelegate,UITableViewDataSource,DDActionSheetViewDelegate,DDLocationMangerDelegate,DDGeocoderMangerDelegate>
...
@implementation DDBuilderAgencySelectVC
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_trainCenterList params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        NSLog(@"**********建造师机构选择列表数据***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if (listArr.count!=0) {
                ...
                for (NSDictionary *dic in listArr) {
                    DDAgencySelectModel *model = [DDAgencySelectModel mj_objectWithKeyValues:dic];
                    model.isClosed=@"1";
...
-(void)assignClick1:(UIButton *)sender{
    ...
    if ([self.isFromeAddApply isEqualToString:@"1"]) {//表明是从添加报名按钮点进来的
        [self judgeHasSignUp:model.train_agency_id andAgencyName:model.name];
    }
    else{
        //跳转到人员信息以及所选机构信息汇总页面
        DDBuildersAndAgencyVC *buildersAndAgency=[[DDBuildersAndAgencyVC alloc]init];
        
        buildersAndAgency.peopleName=self.peopleName;
        buildersAndAgency.majorName=self.majorName;
        buildersAndAgency.certiNo=self.certiNo;
        buildersAndAgency.haveB=self.haveB;
        buildersAndAgency.formal=self.formal;
        buildersAndAgency.endTime=self.endTime;
        buildersAndAgency.endDays=self.endDays;
        buildersAndAgency.tel=self.tel;
        
        buildersAndAgency.agencyName=model.name;
        buildersAndAgency.address=model.detail;
        buildersAndAgency.majorPrice=model.speciality_price;
        buildersAndAgency.price=model.price;
        buildersAndAgency.introduce=model.train_recommend;
        buildersAndAgency.explain=model.train_explain;
        
        buildersAndAgency.userId=self.userId;
        buildersAndAgency.goodsId=model.agency_major_id;
        buildersAndAgency.trainId=model.train_agency_id;
        
        buildersAndAgency.certiTypeId=self.certiTypeId;
        
        buildersAndAgency.idCard=self.idCard;
        buildersAndAgency.companyName=self.companyName;
        buildersAndAgency.companyId=self.companyId;
        
        buildersAndAgency.staffId=self.staffId;
        
        [self.navigationController pushViewController:buildersAndAgency animated:YES];
    }
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_judgeHasSignUp params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (response.isSuccess) {
            ...
            if ([responseObject[KData] isEqual:@1]) {//跳转到之前添过的人的列表
                DDBuilderAddApplyRecordVC *builderAddApplyRecord=[[DDBuilderAddApplyRecordVC alloc]init];
                builderAddApplyRecord.agencyId=agencyId;
                builderAddApplyRecord.agencyName=agencyName;
                [weakSelf.navigationController pushViewController:builderAddApplyRecord animated:YES];
            }
            else{//跳转到填信息页面
                DDBuilderAddApplyVC *builderAddApply=[[DDBuilderAddApplyVC alloc]init];
                builderAddApply.agencyId=agencyId;
                builderAddApply.agencyName=agencyName;
                [weakSelf.navigationController pushViewController:builderAddApply animated:YES];
            }
