...
@implementation DDPerfectPeopleInfoVC
...
-(void)createSubView{
    ...
    if ([DDUtils isEmptyString:self.majorName]) {
        ...
        majorLab.text=[NSString stringWithFormat:@"二级建造师（%@）",self.majorName];
    }
    ...
    [_cardTextField addTarget:self action:@selector(textFieldDidChange:) forControlEvents:UIControlEventEditingChanged];
...
#pragma mark 下一步点击事件
-(void)nextBtnClick{
    if([DDUtils isEmptyString:_cardTextField.text]){
        [DDUtils showToastWithMessage:@"请输入考生身份证号码"];
        return;
    }
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_registerPhoneNumber params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        ...
        if (response.isSuccess) {
            ...
            agencySelectVC.certType = _certType;
            ...
            model.has_b_certificate=self.haveB;
            model.formal=self.formal;
