...
#import "DDTaxIllegalVC.h"
#import "DDLabelUtil.h"
#import "MJRefresh.h"
#import "DDNavigationUtil.h"
#import "DataLoadingView.h"//加载页面
#import "DDNoResult2View.h"//无数据视图
#import "DDTaxIllegalCell.h"//cell
#import "DDTaxIllegalModel.h"//model
#import "DDTaxIllegalDetailVC.h"//税收违法详情

@interface DDTaxIllegalVC ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_taxIllegalList params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"**********税收违法结果数据***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if (listArr.count!=0) {
                ...
                for (NSDictionary *dic in listArr) {
                    DDTaxIllegalModel *model = [[DDTaxIllegalModel alloc]initWithDictionary:dic error:nil];
                ...
                [_noResultView showWithTitle:@"暂无相关税收违法的信息" subTitle:@"去其他地方看看~" image:@"noResult_info"];
            }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    DDTaxIllegalModel *model=_dataSourceArr[indexPath.section];
    
    static NSString * cellID = @"DDTaxIllegalCell";
    DDTaxIllegalCell * cell = (DDTaxIllegalCell *)[tableView dequeueReusableCellWithIdentifier:cellID];
    if (cell == nil) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DDTaxIllegalDetailVC *taxIllegalDetail=[[DDTaxIllegalDetailVC alloc]init];
    taxIllegalDetail.passValue=model.illegalId;
    [self.navigationController pushViewController:taxIllegalDetail animated:YES];
}
