//
//  DDLiveManagerNewAgencySelectVC.m
//  GongChengDD
//
...
#import "DDLiveManagerNewAgencySelectVC.h"
#import "MJRefresh.h"
...
#import "DDLiveManagerNewAddApplyVC.h"//现场管理员新培添加报名页面
#import "DDLiveManagerNewAddApplyRecordVC.h"//添加报名记录页面

@interface DDLiveManagerNewAgencySelectVC ()<UITableViewDelegate,UITableViewDataSource,DDActionSheetViewDelegate,DDLocationMangerDelegate,DDGeocoderMangerDelegate>
...
@implementation DDLiveManagerNewAgencySelectVC
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_trainCenterList params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        NSLog(@"**********现场管理员新培机构选择列表数据***************%@",responseObject);
...
-(void)judgeHasSignUp:(NSString *)agencyId andAgencyName:(NSString *)agencyName{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_judgeHasSignUp params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"***********判断是否存在现场管理员新培报名记录***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if ([responseObject[KData] isEqual:@1]) {//跳转到之前添过的人的列表
                DDLiveManagerNewAddApplyRecordVC *liveManagerNewAddApplyRecord=[[DDLiveManagerNewAddApplyRecordVC alloc]init];
                liveManagerNewAddApplyRecord.agencyId=agencyId;
                liveManagerNewAddApplyRecord.agencyName=agencyName;
                [weakSelf.navigationController pushViewController:liveManagerNewAddApplyRecord animated:YES];
            }
            else{//跳转到填信息页面
                DDLiveManagerNewAddApplyVC *liveManagerNewAddApply=[[DDLiveManagerNewAddApplyVC alloc]init];
                liveManagerNewAddApply.agencyId=agencyId;
                liveManagerNewAddApply.agencyName=agencyName;
                [weakSelf.navigationController pushViewController:liveManagerNewAddApply animated:YES];
            }
