...
#import "DDSafeManNewAddApplyVC.h"
...
#import "DDSafeManNewPayEnsureVC.h"//确认订单页面
...
@interface DDSafeManNewAddApplyVC ()<UITableViewDelegate,UITableViewDataSource,DDActionSheetViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation DDSafeManNewAddApplyVC
...
-(void)createBottomView{
    ...
    [_submitBtn setBackgroundColor:KColorBidApprovalingWait];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {//单位名称
        ...
        companyName.companyBlock = ^(NSString *companyName, NSString *companyId) {
            ...
            if (![DDUtils isEmptyString:_companyName] && ![DDUtils isEmptyString:_name] && ![DDUtils isEmptyString:_majorId] && ![DDUtils isEmptyString:_tel] && ![DDUtils isEmptyString:_idCard]&&[_isCheckSuccess isEqualToString:@"1"]) {
                [_submitBtn setBackgroundColor:kColorBlue];
    ...
    else if(indexPath.section==5){//填写手机号
    ...
    else if(indexPath.section==4){//报名成功截图
        if ([DDUtils isEmptyString:_idCard]) {
            [DDUtils showToastWithMessage:@"请先输入身份证号"];
            return;
        }
        if (@available(iOS 11, *)) {
...
-(void)submitClick{
    ...
    [params setValue:@"2" forKey:@"certType"];
    ...
    [[DDHttpManager sharedInstance]sendPostRequest:KHttpRequest_builderAddApply params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        ...
        if (response.isSuccess) {
            [hud hide:NO];
            DDSafeManNewPayEnsureVC *safeManNewPayEnsure=[[DDSafeManNewPayEnsureVC alloc]init];
            safeManNewPayEnsure.userId=_userId;
            safeManNewPayEnsure.goodsId=_goodsId;
            safeManNewPayEnsure.trainId=self.agencyId;
            
            safeManNewPayEnsure.certiTypeId=_certiTypeId;
            safeManNewPayEnsure.certiId=[NSString stringWithFormat:@"%@",responseObject[KData]];
            
            safeManNewPayEnsure.peopleName=_name;
            safeManNewPayEnsure.majorName=_majorName;
            safeManNewPayEnsure.agencyName=self.agencyName;
            safeManNewPayEnsure.majorPrice=_price;
            safeManNewPayEnsure.companyName = _companyName;
            safeManNewPayEnsure.isFromeAddApply = _isFromeAddApply;
            [self.navigationController pushViewController:safeManNewPayEnsure animated:YES];
        }
        else{
...
-(void)comparePicture{
    NSMutableDictionary * params = [[NSMutableDictionary alloc] initWithCapacity:3];
    [params setValue:@[_idCard] forKey:@"data_list"];
    ...
    [[DDHttpManager sharedInstance]sendPostRequest:KHttpRequest_comparePictures params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        ...
        [hud hide:YES afterDelay:KHudShowTimeSecound];
        if (response.isSuccess) {
            if ([response.data isEqual:@0]) {//比对失败
                _isCheckSuccess=@"0";
