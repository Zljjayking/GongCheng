...
#import "DDSafeManNewPayEnsureVC.h"
...
#import "DDBuilderPaySucceedVC.h" //支付成功页面
...
@interface DDSafeManNewPayEnsureVC ()<UITableViewDelegate,UITableViewDataSource,CNContactPickerDelegate,DDCitySelectPickerViewDelegate,DDPayMethodViewDelegate,PlaceholdTextViewDelegate>
...
@implementation DDSafeManNewPayEnsureVC
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            ...
            if ([DDUtils isEmptyString:self.peopleName] && [DDUtils isEmptyString:self.majorName]) {
                ...
                NSString * totalString = [NSString stringWithFormat:@"%@（%@）",self.peopleName,self.majorName];
                
                NSMutableAttributedString * attributedString =  [DDUtils adjustTextColor:totalString rangeText:[NSString stringWithFormat:@"（%@）",self.majorName] color:KColorGreySubTitle];
                
                attributedString = [DDUtils changeTextFontWithOriginalText:totalString needText:[NSString stringWithFormat:@"（%@）",self.majorName] needTextFont:kFontSize26];
                cell.titleLab2.attributedText = attributedString;
            }
        }
...
-(void)actionsheetDisappearAliPay:(DDPayMethodView *)actionSheet{
    ...
    [params setValue:@"2" forKey:@"goodsType"];
...
-(void)createPaySuccessView{
    ...
    succeedVC.vcName=@"DDSafeManNewAgencySelectVC_DDSafeManNewAddApplyRecordVC";
