...
#import "DDArchitectDetailInfoVC.h"
#import "DataLoadingView.h"
#import "NBLScrollTabController.h"//多页面滚动视图工具
#import "DDArchitectReceivedProjectsVC.h"//承接项目页面
...
@interface DDArchitectDetailInfoVC ()<NBLScrollTabControllerDelegate,DDArchitectProjectsHeaderViewDelegate,MFMessageComposeViewControllerDelegate>

{
    UIView *_titleView;
}
@property (nonatomic,strong) DDArchitectProjectsHeaderView * topView;//顶部视图
...
@implementation DDArchitectDetailInfoVC
...
-(void)shareClick{
    ...
    [UMSocialUIManager showShareMenuViewInWindowWithPlatformSelectionBlock:^(UMSocialPlatformType platformType, NSDictionary *userInfo) {
        ...
        if (platformType == UMSocialPlatformType_Sms) {
            ...
            // 设置短信内容
            _messageController.body = [NSString stringWithFormat:@"下载工程点点,查看人员证书信息、变更记录及个人工程业绩。http://m.koncendy.com/pages/StaffDetail/main?specialityCode=%@&certId=%@&id=%@&name=%@",_specialCodeStr,_certiId,_staffInfoId,_nameStr];
            ...
            //设置网页地址
            shareObject.webpageUrl =[NSString stringWithFormat:@"http://m.koncendy.com/pages/StaffDetail/main?specialityCode=%@&certId=%@&id=%@&name=%@",_specialCodeStr,_certiId,_staffInfoId,_nameStr];
...
- (void)createTopView{
    ...
    _topView = [[[NSBundle mainBundle] loadNibNamed:@"DDArchitectProjectsHeaderView" owner:self options:nil] firstObject];
    _topView.frame = CGRectMake(0,0, Screen_Width, 105);
...
-(void)requestDetailInfo{
    ...
    [[DDHttpManager sharedInstance]sendPostRequest:KHttpRequest_bulderDetailInfo params:params success:^(NSURLSessionDataTask *operation, id responseObject){
        NSLog(@"**********建筑师等五类人员详情数据结果***************%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            //标题
            //3一级结构师 4二级结构师 5化工工程师列表 6一级建筑师 7二级建筑师
            NSString *subTitleStr;
            if ([weakSelf.type isEqualToString:@"3"]) {
                subTitleStr=@"一级结构师";
            }
            else if ([weakSelf.type isEqualToString:@"4"]) {
                subTitleStr=@"二级结构师";
            }
            else if ([weakSelf.type isEqualToString:@"5"]) {
                subTitleStr=@"化工工程师";
            }
            else if ([weakSelf.type isEqualToString:@"6"]) {
                subTitleStr=@"一级建筑师";
            }
            else if ([weakSelf.type isEqualToString:@"7"]) {
                subTitleStr=@"二级建筑师";
            }
...
-(void)createScrollView{
    ...
    _scrollTabController.view.frame = CGRectMake(0, 105+15, Screen_Width, Screen_Height);
...
- (NSArray *)viewControllers{
    if (!_viewControllers) {
        ...
        DDArchitectReceivedProjectsVC *vc1 = [[DDArchitectReceivedProjectsVC alloc] init];
...
#pragma mark DDArchitectProjectsHeaderViewDelegate
