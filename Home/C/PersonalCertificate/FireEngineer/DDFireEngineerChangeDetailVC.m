//
//  DDFireEngineerChangeDetailVC.m
//  GongChengDD
//
...
#import "DDFireEngineerChangeDetailVC.h"
...
@interface DDFireEngineerChangeDetailVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation DDFireEngineerChangeDetailVC
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"变更记录";
...
- (void)requestData{
    ...
    [[DDHttpManager sharedInstance]sendGetRequest:KHttpRequest_fireEngineerChangeDetail params:params success:^(NSURLSessionDataTask *operation, id responseObject) {
        NSLog(@"消防工程师变更记录%@",responseObject);
        ...
        if (response.isSuccess) {
            ...
            if (![DDUtils isEmptyString:totalCountString]) {
                ...
                NSMutableAttributedString * attributedString =  [DDUtils adjustTextColor:totalString rangeText:totalCountString color:KColorBlackTitle];
                _headerLabel.attributedText = attributedString;
            }else{
                NSString * totalString = @"共0条";
                NSMutableAttributedString * attributedString =  [DDUtils adjustTextColor:totalString rangeText:@"0" color:KColorBlackTitle];
                _headerLabel.attributedText = attributedString;
            }
