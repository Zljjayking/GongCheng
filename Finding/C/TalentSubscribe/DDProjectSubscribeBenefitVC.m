...
#import "DDProjectSubscribeBenefitVC.h"
#import "DDProjectSubscribeVC.h"//中标监控页面

@interface DDProjectSubscribeBenefitVC ()

@end

@implementation DDProjectSubscribeBenefitVC
...
-(void)createUI{
    ...
    scrollView.contentSize=CGSizeMake(Screen_Width, 15+936.5*Scale+15);
    [self.view addSubview:scrollView];
    
    UIImageView *imgView=[[UIImageView alloc]initWithFrame:CGRectMake(0, 15, Screen_Width, 936.5*Scale)];
    imgView.backgroundColor=kColorWhite;
    if ([self.isCallBidding isEqualToString:@"1"]) {
        imgView.image=[UIImage imageNamed:@"finding_zhaobiao"];
    }
    else{
        imgView.image=[UIImage imageNamed:@"finding_zhongbiao"];
    }
...
-(void)goSettingClick{
    ...
    projectSubscribe.isCallBidding=self.isCallBidding;
    [self.navigationController pushViewController:projectSubscribe animated:YES];
}
